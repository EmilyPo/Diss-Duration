---
title: "More Data for Duration Sims- Full Age Range"
...
SOME WILL NEVER HAVE HETEROSEXUAL RELATIONS
...
cohab_mean_y <- 1/exp(coef(expFit)[[1]])
spouse_mean_y <- cohab_mean*(1/exp(coef(expFit)[[2]]))
other_mean_y <- cohab_mean*(1/exp(coef(expFit)[[3]]))

means <- as.data.frame(rbind(cohab_mean_y, spouse_mean_y, other_mean_y))
means[,1] <- means[,1]/12 
colnames(means) <- c("Mean Years")

dat2 <- readRDS("~/NSFG_DATA/Objects/alters_active.rds")
datsvy2 <- as_survey_design(dat2 ,weights = weight, ids=ego)

means2 <-  datsvy2 %>% group_by(reltype) %>% 
...
don't actually need this network for duration simulations 
